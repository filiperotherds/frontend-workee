...
export function ArchiveButton({ estimateId }: archiveButtonProps) {
  return (
    <AlertDialog>
      <Tooltip>
        ...
        <AlertDialogContent className="sm:max-w-md rounded-2xl p-4">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel asChild>
              <Button className="h-8 text-muted-foreground">Cancelar</Button>
            </AlertDialogCancel>
            <AlertDialogAction asChild>
              <Button
                variant={"ghost"}
                className="h-8 bg-red-50 hover:bg-red-100 text-destructive hover:text-destructive"
              >
                <Archive />
                Arquivar
              </Button>
